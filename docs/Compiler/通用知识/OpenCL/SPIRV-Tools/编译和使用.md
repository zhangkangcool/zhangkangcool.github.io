...
将下载好的`SPIRV-Tools`放于llvm/tools/spirv-tools。`spirv-tools`是从llvm18开始集成到llvm中的。
...
LLVM17已经集成了该项目编译LLVM时需要打开SPIRV编译，这样`llc --version`时会出现`spirv32     - SPIR-V 32-bit, spirv64     - SPIR-V 64-bit`。但依然没有llvm-tools
