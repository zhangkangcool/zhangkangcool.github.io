在 OpenCL 的 ICD 架构中，当系统存在多个`.icd`文件时，**编译阶段不关心具体使用哪个驱动**（因为 ICD 编译统一链接到系统目录下的`libOpenCL.so`,如/usr/lib/libOpenCL.so），而**运行时的驱动选择**由 ICD Loader 和应用程序共同决定。下面详细说明选择机制和控制方法：
